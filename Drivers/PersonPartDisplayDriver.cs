...
namespace Lombiq.TrainingDemo.Drivers
{
    ...
    public class PersonPartDisplayDriver : ContentPartDisplayDriver<PersonPart>
    {
        ...
        public override IDisplayResult Display(PersonPart part) =>
            ...
            // using Placement.json files. Since this is not possible to put comments in a .json file the explanation
            // is here but make sure you check the file while you reading this. It's important to give a location
            // somewhere otherwise to shape won't be displayed.
            
            // In Placement files you can give specific locations to any shapes generated by Orchard. You also can give
            // rules the location will be activated only if it matches. In our Placement file you can see that the
            // PersonPart shape gets the first position in the Content zone and the TextField shape gets the second. To
            // make sure that not all the TextFields will get the same position a "differentiatior" property is given
            // which refers to the part name where the field is attached to and the field name. Make sure you also read
            // the documentation to know this feature better:
            // https://orchardcore.readthedocs.io/en/latest/OrchardCore/OrchardCore.DisplayManagement/README/#placement-files

            // NEXT STATION: Placement.json
