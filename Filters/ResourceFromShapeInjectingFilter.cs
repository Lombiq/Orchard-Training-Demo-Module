...
namespace Lombiq.TrainingDemo.Filters
{
    public class ResourceFromShapeInjectingFilter : IAsyncResultFilter
    {
        ...
        public async Task OnResultExecutionAsync(ResultExecutingContext context, ResultExecutionDelegate next)
        {
            ...
            // We use the shape factory again to instantiate the AlertScriptShape. Check out
            // Views/AlertScriptShape.cshtml because there's something curious there too, then come back!
            // Next, we also use the display helper to execute the shape and generate its HTML content.
            var shapeContent = await _displayHelper.ShapeExecuteAsync(await _shapeFactory.New.AlertScriptShape());

            // Did you know that you can inject inline scripts with resource manager too? You can use this technique
            // not just like this to generate inline scripts but also e.g. to generate HTML output from shapes in
            // background tasks (like when sending e-mails and you want to have proper templates for them).
