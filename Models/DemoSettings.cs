namespace Lombiq.TrainingDemo.Models
{
    ...
    // Core Options pattern to also register it as an option too. This will allow us to not only store these settings
    // in site settings but provide them from a huge variety of other means too, like from an appsettings.json file or
    // environment variables. For all these see: https://docs.microsoft.com/en-us/aspnet/core/fundamentals/configuration/
    // If you're not familiar with Options check out the official docs here:
